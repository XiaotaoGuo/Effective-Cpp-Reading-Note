...
void runConstPointerExample() {
    ...
    *p_c_nc = 'a';                        // works
    ...
    // *p_c_c = 'a';       // error: assignment of read-only location ‘*(const char*)p_c_c’
}

void runConstSTLExample() {

    std::vector<int> testVec(10, 0);
    const std::vector<int>::iterator iter = testVec.begin();    // const pointer, non-const data;
    // iter = testVec.end();     // error: passing ‘const iterator {aka const __gnu_cxx::__normal_iterator<int*, std::vector<int> >}’ as ‘this’ argument discards qualifiers [-fpermissive]
    *iter = 10;                                                 // works
    std::vector<int>::const_iterator cIter = testVec.begin();   // non-const pointer, const data;
    ++cIter;                                                    // works;  
    // *cIter = 10;    // error: assignment of read-only location ‘cIter.__gnu_cxx::__normal_iterator<const int*, std::vector<int> >::operator*()’               
